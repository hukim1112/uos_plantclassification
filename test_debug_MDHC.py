...
from test_models import MDHC_debug1, MDHC_debug2
from test_models import MHLN_debug
...
def test_modified_MDHC_debug1(log_dir, finetune, metric_func, beta, device):
    ...
    model = MDHC_debug1(loss_fn=MHLN_debug(new_fine_to_coarse, beta=beta, device=device), baseline=baseline, num_classes=[len(test_genera), len(species_list)]).to(device)
...
def test_modified_MDHC_debug2(log_dir, finetune, metric_func, beta, device):
    os.makedirs(log_dir, exist_ok=True)
    weight_path = f"{log_dir}/checkpoint.pt"
    ...
    model = MDHC_debug2(loss_fn=MHLN_debug(new_fine_to_coarse, beta=beta, device=device), baseline=baseline, num_classes=[len(test_genera), len(species_list)]).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('model_type', type=str,
                        metavar='model_type',
                        choices=["debug1","debug2"],
                        help='What is the debug model type?')    
