...
from experiment.plantnet import baseline, genera, genera_species_hierarchical_classifier

def train_EfficientB4(device):
    exp_list = [genera]
    exp_name = ["genera"]
    for exp, name in zip(exp_list, exp_name):
        exp_dir = f"/home/files/experiments/plantnet/baseline/EfficientB4/{name}"
...
def train_VGG19(device):
    ...
    exp_name = ["exp_set1"]
    for exp, name in zip(exp_list, exp_name):
        exp_dir = f"/home/files/experiments/plantnet/baseline/VGG19/{name}"
...
def train_ResNet101(device):
    ...
    exp_name = ["exp_set1"]
    for exp, name in zip(exp_list, exp_name):
        exp_dir = f"/home/files/experiments/plantnet/baseline/ResNet101/{name}"
...
def train_WideResNet101_2(device):
    ...
    exp_name = ["exp_set1"]
    for exp, name in zip(exp_list, exp_name):
        exp_dir = f"/home/files/experiments/plantnet/baseline/WideResNet101_2/{name}"
        exp(exp_dir, WideResNet101_2, device)

def train_genera_species_hierarchical_classifier(device, method):
    exp_dir = f"/home/files/experiments/plantnet/genera_species_hierarchical_classifier"
    baseline_weight_path = "/home/files/experiments/plantnet/baseline/EfficientB4/exp1/checkpoints/checkpoint.pt"
