...
class ResNet101(Baseline):
    def __init__(self, num_classes, loss_fn, fc_type="shallow"):
        super(ResNet101, self).__init__(loss_fn)
    ...
    def get_patch_embedding(self):
        cnn = timm.create_model('resnet101', pretrained=False)
    ...
    def get_fc(self):
        if self.fc_type == 'deep':
            ...
            fc = nn.Linear(2048, self.num_classes)
        else:
            raise ValueError(f"Wrong fc-type input {self.fc_type}")
        return fc

class WideResNet101_2(Baseline):
    def __init__(self, num_classes, loss_fn, fc_type="shallow"):
        super(WideResNet101_2, self).__init__(loss_fn)
        self.num_classes = num_classes
        self.fc_type = fc_type
        self.patch_embedding = self.get_patch_embedding()
        self.embedding = self.get_embedding()
    ...
    def get_patch_embedding(self):
        cnn = timm.create_model('wide_resnet101_2', pretrained=False)
        return nn.Sequential( *list(cnn.children())[:-2])
