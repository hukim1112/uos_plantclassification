...
from test_models import modified_DHC
from test_models import modified_HLN
...
def test_modified_DHC(log_dir, finetune, metric_func, beta, device):
    os.makedirs(log_dir, exist_ok=True)
    weight_path = f"{log_dir}/checkpoint.pt"
    ...
    model = modified_DHC(loss_fn=HierarchicalLossNetwork(new_fine_to_coarse, beta=beta, device=device), baseline=baseline, num_classes=[len(test_genera), len(species_list)]).to(device)
...
def test_modified_DLN(log_dir, finetune, metric_func, beta, device):
    os.makedirs(log_dir, exist_ok=True)
    weight_path = f"{log_dir}/checkpoint.pt"
    ...
    model = HierarchicalClassifier(loss_fn=modified_HLN(new_fine_to_coarse, beta=beta, device=device), baseline=baseline, num_classes=[len(test_genera), len(species_list)]).to(device)
...
def test_modified_DHC_and_DLN(log_dir, finetune, metric_func, beta, device):
    os.makedirs(log_dir, exist_ok=True)
    weight_path = f"{log_dir}/checkpoint.pt"
    ...
    model = modified_DHC(loss_fn=modified_HLN(new_fine_to_coarse, beta=beta, device=device), baseline=baseline, num_classes=[len(test_genera), len(species_list)]).to(device)
...
if __name__ == "__main__":
    device = "cuda:2"
    test_name = "comparable_classifier_all_genera"
    finetune = False
    metric_func = metric_to_acc
    
    #flat classifier
    log_dir = f"/home/files/experiments/ablation_test/{test_name}/exp1_flat_classifier/"
    test_flat_classifier(log_dir, finetune, metric_func, device)
    
    #DHC with beta = 0.8
    log_dir = f"/home/files/experiments/ablation_test/{test_name}/exp2_DHC_beta0.8/"
    beta = 0.8
    test_hierarchical_classifier(log_dir, finetune, metric_func, beta, device)   
    
    #DHC with beta scheduled
    log_dir = f"/home/files/experiments/ablation_test/{test_name}/exp3_DHC_beta_scheduled/"
    test_hierarchical_classifier(log_dir, finetune, metric_func, beta="scheduler", device=device) 


    #modified DHC with beta=0.8
    log_dir = f"/home/files/experiments/ablation_test/{test_name}/exp4_modified_DHC/"
    test_modified_DHC(log_dir, finetune, metric_func, beta=0.8, device=device)    
    
    #modified DLN with beta=0.8
    log_dir = f"/home/files/experiments/ablation_test/{test_name}/exp5_modified_DLN/"
    test_modified_DLN(log_dir, finetune, metric_func, beta=0.8, device=device)    
    
    #modified DLN with beta=0.8
    log_dir = f"/home/files/experiments/ablation_test/{test_name}/exp6_modified_DHC_and_DLN/"
    test_modified_DHC_and_DLN(log_dir, finetune, metric_func, beta=0.8, device=device)   
